...
def display_logo(url, row, column,factor, container, padx=20, pady=40):
    ...
    img_label = Label(container, image=img, bg="#00A6FF")
    img_label.image = img
...
def generate_child_indvidual(child,document_type, generation_type):
    child.geometry("750x250")
    child.title(f"Generador individual {document_type} - {generation_type}")

def gemerate_window(parent, document_type, generation_type):
    # Create a child window using Toplevel method
    child_individual = Toplevel(parent)
    generate_child_indvidual(child_individual, document_type, generation_type)
